# This module will do the spark heavy lifting transformations
# We use struct() function  to group multiple existing columns together
# into a single, nested structure column (a StructType).
...
def join_party_address(p_df, a_df):
    return (
        p_df.join(a_df, "party_id", "left_outer")
        .groupBy("account_id")
        .agg(
            collect_list(
                struct(
...
def apply_header(spark, df):
    ...
    header_df = spark.createDataFrame(header_info).toDF(
        "eventType", "majorSchemaVersion", "minorSchemaVersion"
    )
    ...
    event_df = (
        ...
        .select(
            struct(
                ...
                lit(date_format(current_timestamp(), "yyyy-MM-dd'T'HH:mm:ssZ")).alias(
                    "eventDateTime"
                ),
            ).alias("eventHeader"),
            array(
